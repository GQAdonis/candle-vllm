...
impl LogitsProcessor {
    pub fn from_sampling(seed: u64, sampling: Sampling) -> Self {
        ...
        Self {
            ...
            sampling,
            #[cfg(feature = "cutlass")]
            fast_sampler: Arc::new(std::sync::Mutex::new(
                crate::attention::sampler::Sampler::new(),
            )),
        }
    }
    ...
    pub fn sample(
        ...
        // CUDA fast path for supported sampling strategies (requires attention-rs >= 0.3 with sampler module)
        #[cfg(feature = "cutlass")]
        {
