    ...
    >(
        ...
        match (src.device(), dst.device()) {
                ...
                for (src_block_number, dst_block_number) in block_mapping {
                    ...
                    unsafe {
                        result::memcpy_htod_async(
                            dst_base_ptr.wrapping_add(dst_offset),
            ...
            (Device::Cuda(src_dev), Device::Cpu) => {
                ...
                let dst_ptr = dst_slice.as_ptr() as *mut u8;
                ...
                for (src_block_number, dst_block_number) in block_mapping {
                    ...
                    unsafe {
                        result::memcpy_dtoh_async(
                            host_dst,
                            src_base_ptr.wrapping_add(src_offset),
                            src_dev.cu_stream(),
                        )
                        .map_err(candle_core::Error::wrap)?;
                    }
            ...
            (Device::Cuda(src_dev), Device::Cuda(dst_dev)) => {
                ...
                for (src_block_number, dst_block_number) in block_mapping {
                    ...
                    assert!(
                        src_offset_elem + block_size_elements <= src.elem_count(),
