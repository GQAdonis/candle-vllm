...
#[cfg(feature = "cuda")]
pub use candle_vllm_kernels as kernels;

#[cfg(feature = "metal")]
pub use candle_vllm_metal_kernels as metal_kernels;
        ...
        if let Some(fm) = input_metadata.flashinfer_metadata.as_ref() {
            ...
            if flashinfer_group_supported {
                ...
                if let (Some(kc), Some(vc)) = (key_cache.as_ref(), value_cache.as_ref()) {
                    crate::attention::flashinfer::append_kv_cache(
                        &key,
                        &value,
                        kc,
                        vc,
                ...
                return if input_metadata.is_prefill {
                    crate::attention::flashinfer::prefill(
                        &query,
                        key_cache.as_ref().unwrap(),
                        value_cache.as_ref().unwrap(),
                        self.k_scale.as_ref(),
                    ...
                    crate::attention::flashinfer::decode_with_plan(
                        &query,
                        key_cache.as_ref().unwrap(),
                        value_cache.as_ref().unwrap(),
                        self.k_scale.as_ref(),
