...
impl GPTQMatMul {
    ...
    >(
        ...
        let stream_ref: &candle::cuda_backend::cudarc::driver::CudaStream = &stream;
        ...
        // Extract raw pointers in a block scope so guards are dropped before `out` is moved.
        {
            let (out_raw, _out_g) = out.device_ptr(stream_ref);
            let out_ptr = out_raw as *mut c_void;
            let (in_raw, _in_g) = input.device_ptr(stream_ref);
            let in_ptr = in_raw as *const c_void;
            let (qw_raw, _qw_g) = qw.device_ptr(stream_ref);
            let qw_ptr = qw_raw as *const c_void;
            let (qs_raw, _qs_g) = qs.device_ptr(stream_ref);
            let qs_ptr = qs_raw as *const c_void;
            ...
            let qzeros_ptr = if self.qzeros.is_some() {
                ...
                let (qz_raw, _qz_g) = qzeros_.device_ptr(stream_ref);
                qz_raw as *const c_void
            } else {
                std::ptr::null()
            };
            ...
            let g_idx_ptr = if self.g_idx.is_some() {
                ...
                let (gi_raw, _gi_g) = g_idx_.device_ptr(stream_ref);
                gi_raw as *const c_void
            } else {
                std::ptr::null()
            };
            ...
            unsafe {
                if marlin_format {
                    let workspace_ptr = if self.workspace.is_some() {
                        ...
                        let (ws_raw, _ws_g) = workspace_.device_ptr(stream_ref);
                        ws_raw as *const c_void
                    } else {
...
impl MarlinRepack {
    ...
    >(
...

        {
            ...
            let (q_raw, _q_g) = q.device_ptr(stream_ref);
